...
class PathAggNet(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, num_relation, query_input_dim, message_func="distmult",
        ...
        self.relation_linear = nn.Linear(query_input_dim, num_relation * input_dim) #1*d -> n*d

    def forward(self, input, query, initial_stat, edge_index, edge_type, size, r_index, edge_weight=None):
        batch_size = len(query)
        relation = self.relation_linear(query).view(batch_size, self.num_relation, self.input_dim)

        # dimension = (batch_size, self.num_users, self.input_dim)
